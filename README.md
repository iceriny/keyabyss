...
`npm run dev` / `npm run serve` 启动 Vite。`build` 先执行内容校验、依赖边界校验和严格 TypeScript 检查，再通过 `vite-plugin-singlefile` 内联应用代码与样式，写入许可证，并校验 `index.html`、词库脚本、原生战场脚本及 public 静态资源的 SHA-256。生产文件可从 `file://` 直接打开。
...
vite.config.mjs            React + 内联应用 / 外置资产构建配置
```
...
导入在本机解析。仅主动点击「同步源词表」时联网；默认无网络请求、CDN、远程字体或外部图片。设置、词库和最近 30 场战报存于 localStorage，刷新结束当前战局。存储不足时自定义词库仍可在本次会话使用，并可导出备份。
